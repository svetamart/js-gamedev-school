...
function addSpecialEffects(tilesToDelete) {
	var visited = Array(xTileCount).fill().map(() => new Array(yTileCount).fill(false));
	var tilesToAdd = [];
	var links = [];

	for (let tile of tilesToDelete) {
		if (tiles[tile.x][tile.y] === 2) {
			links = [{ x: 0, y: 1 }, { x: 0, y: -1 }, { x: 1, y: 0 }, { x: -1, y: 0 }];
			processTiles(tile.x, tile.y, links);
		} 
		else if (tiles[tile.x][tile.y] === 1) {
			links = [{ x: -1, y: 0 }, { x: 1, y: 0 }, { x: 0, y: -1 }, { x: 0, y: 1 },
				{ x: -2, y: 0 }, { x: 2, y: 0 }, { x: 0, y: -2 }, { x: 0, y: 2 }];
			processTiles(tile.x, tile.y, links);
		}
		else if (tiles[tile.x][tile.y] === 4) {
			yellowTiles(tile.x, tile.y);
		}
	}

	function yellowTiles(currentX, currentY) {
		var links = [{ x: 0, y: 1 }, { x: 0, y: -1 }, { x: 1, y: 0 }, { x: -1, y: 0 }];

		for (let link of links) {
			let x = currentX + link.x;
			let y = currentY + link.y;

			if (isCorrectTile(x, y) && !visited[x][y] && tiles[x][y] === 4) {
				visited[x][y] = true;
				tilesToAdd.push({ x: x, y: y });
				yellowTiles(x, y);
	...
	function processTiles(currentX, currentY, links) {
		for (let link of links) {
			let x = currentX + link.x;
			let y = currentY + link.y;
